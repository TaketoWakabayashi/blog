...
とはいえ組織全体でより大きな効果を生み出すには、個々人が都度手元で処理するのではなく、共通目的の処理を横断的に集約し、前処理済みデータを配布することで、出力品質と処理効率を高める必要があります。
...
なお、現時点では BigQuery のオプティマイザはAI関数を呼び出す処理を高コストな処理だと認識しきれていないようで、WHERE句やLIMIT句で対象レコードをフィルタするようなクエリを繋げて書くと、先に全件処理されてからフィルタされるような挙動をしてしまうことがあります。
そのため、オプティマイザの影響を受けないように必ず一時テーブルを作成するなどの対策が必要になります。
